...
export function formatCurrency(amount, opts={}) {
  ...
  if (Math.abs(amount)>1e15) amount=Math.sign(amount)*1e15;
  const rounded=Math.round(amount*100)/100;
  return new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',minimumFractionDigits:decimals,maximumFractionDigits:decimals}).format(rounded);
}
export function formatPercent(value,decimals=2){
  if (!isFinite(value) || value==null) return "0%";
  const clamped=Math.max(-100,Math.min(200,value));
...
export function allocatePeriods(annual,periods){
  ...
  while(remainderCents>0){
    allocations[i%periods]=roundCents(allocations[i%periods]+0.01);
    remainderCents--;i++;
  }
  while(remainderCents<0){
    allocations[i%periods]=roundCents(allocations[i%periods]-0.01);
