...
export function sanitizeInputs(raw={}){
  ...
  out.isBlind=!!raw.isBlind; out.age65=!!raw.age65;
  ...
  const rw=raw.w4||{};
  out.w4={
    multipleJobs:!!(rw.step2Checkbox??rw.multipleJobs), step2Checkbox:!!(rw.step2Checkbox??rw.multipleJobs),
    dependentAmount:cleanNumber(rw.dependentAmount??rw.step3Amount??0), step3Amount:cleanNumber(rw.step3Amount??rw.dependentAmount??0),
    otherIncome:cleanNumber(rw.otherIncome??rw.step4aOtherIncome??0), step4aOtherIncome:cleanNumber(rw.step4aOtherIncome??rw.otherIncome??0),
    deductions:cleanNumber(rw.deductions??rw.step4bDeductions??0), step4bDeductions:cleanNumber(rw.step4bDeductions??rw.deductions??0),
    extraWithholding:cleanNumber(rw.extraWithholding??rw.step4cExtraWithholding??0), step4cExtraWithholding:cleanNumber(rw.step4cExtraWithholding??rw.extraWithholding??0),
    exempt:!!rw.exempt
  };
...
export function validateInputs(sanitized){
  const errors=[]; const warnings=[]; const {grossAnnual,hoursPerWeek,weeksPerYear,filingStatus,state}=sanitized;
  if(!Number.isFinite(grossAnnual)||Number.isNaN(grossAnnual)) errors.push({field:'grossAnnual',message:'Salary must be a valid number'});
  else if(grossAnnual<0) errors.push({field:'grossAnnual',message:'Salary cannot be negative'});
  else if(grossAnnual>1e12) errors.push({field:'grossAnnual',message:'Salary exceeds the $1T maximum'});
  ...
  if(hoursPerWeek<0||hoursPerWeek>168) errors.push({field:'hoursPerWeek',message:'Hours per week must be between 0 and 168'});
  if(weeksPerYear<0||weeksPerYear>52) errors.push({field:'weeksPerYear',message:'Weeks per year must be between 0 and 52'});
  const d=sanitized.deductions;
  const totalPretax=(d.traditional401k||0)+(d.hsa||0)+(d.healthPremiums||0)+(d.fsa||0)+(d.otherPretax||0);
  if(totalPretax>grossAnnual&&grossAnnual>0) warnings.push({field:'deductions',message:'Pre-tax deductions exceed gross and will be clamped'});
  if(d.traditional401kPercent<0||d.traditional401kPercent>100) errors.push({field:'traditional401kPercent',message:'401(k) percentage must be between 0 and 100'});
  if(!['self','family'].includes(d.hsaCoverage)) errors.push({field:'hsaCoverage',message:'HSA coverage must be self or family'});
  for(const [k,v] of Object.entries(d)){if(typeof v==='number'&&!Number.isFinite(v))errors.push({field:k,message:`Invalid deduction ${k}`});if(typeof v==='number'&&v<0)errors.push({field:k,message:`Deduction ${k} cannot be negative`});}
  for(const [k,v] of Object.entries(sanitized.federalDeductions)) if(typeof v==='number'&&(!Number.isFinite(v)||v<0)) errors.push({field:k,message:`Invalid federal deduction ${k}`});
  const w=sanitized.w4;
  for(const [k,v] of Object.entries({step3Amount:w.step3Amount,step4aOtherIncome:w.step4aOtherIncome,step4bDeductions:w.step4bDeductions,step4cExtraWithholding:w.step4cExtraWithholding})) if(!Number.isFinite(v)||v<0) errors.push({field:k,message:`${k} must be zero or greater`});
...
export function normalizeIncome(sanitized){
  let grossAnnual=sanitized.grossAnnual; const freq=sanitized.payFrequency;
  ...
  if(!Number.isFinite(grossAnnual)||grossAnnual<0) grossAnnual=0;
