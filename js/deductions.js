...
export function classifyDeductions(sanitized, stateAbbr){
  ...
  let hsa=sanitized.deductions.hsa||0;
  ...
  if (trad401kPercent>0 && gross>0){
    const pctAmount=(trad401kPercent/100)*gross;
    trad401k=Math.max(trad401k,pctAmount);
  }
  ...
  const hsaCoverage=sanitized.deductions.hsaCoverage||"self";
  let hsaLimit=hsaCoverage==="family"?limits.hsa.family:limits.hsa.selfOnly;
  if (age>=55) hsaLimit+=limits.hsa.catchUp55;
  const requested401k=trad401k;
  const requestedHSA=hsa;
  const applied401k=Math.min(trad401k,applicable401kLimit,gross);
  const appliedHSA=Math.min(hsa,hsaLimit,gross);
  const capped401k=requested401k>applicable401kLimit;
  const cappedHSA=requestedHSA>hsaLimit;
  const hsaViaPayroll=true;
  ...
  const breakdown={
    ...
    hsa:{requested:requestedHSA,applied:appliedHSA,capped:cappedHSA,limit:hsaLimit,coverage:hsaCoverage,viaPayroll:hsaViaPayroll,reduces:{federal:true,socialSecurity:hsaViaPayroll,medicare:hsaViaPayroll,state:stateAllowsHSA}},
  ...
  const totals={pretaxFedOnly:0,pretaxFedFica:0,pretaxState:0,totalPretax:0};
  ...
  totals.totalPretax=breakdown.traditional401k.applied+breakdown.hsa.applied+breakdown.healthPremiums.applied+breakdown.fsa.applied+breakdown.otherPretax.applied+breakdown.postTax.applied;
  return {
    breakdown,
    totals,
    limits:{"401k":{requested:requested401k,applied:applied401k,capped:capped401k,limit:applicable401kLimit},hsa:{requested:requestedHSA,applied:appliedHSA,capped:cappedHSA,limit:hsaLimit,coverage:hsaCoverage}},
    warnings:[...(capped401k?[ `401(k) contribution capped to $${applicable401kLimit} legal limit`]:[]),...(cappedHSA?[ `HSA contribution capped to $${hsaLimit} (${hsaCoverage} coverage)`]:[]),...((totals.pretaxFedOnly+totals.pretaxFedFica)>gross && gross>0?["Total pre-tax deductions exceed gross and were clamped"]:[])]
  };
}
